...
if TYPE_CHECKING:
    ...
    def type_check(ssh: SSHHost) -> None:
        _: Host = ssh
...
class SSHHost:
    """
    A Host implementation that interacts with a remote machine via SSH and SFTP.
    """

    name: str = "ssh"

    class Process:
        """Host process wrapper around asyncssh.SSHClientProcess."""
        ...
        async def wait(self) -> int:
            ...
            # stdout/stderr remain readable after wait, matching LocalHost.
    ...
    def normpath(self, path: StrOrHostPath) -> HostPath:
        return HostPath(posixpath.normpath(str(path)))

    def gethome(self) -> HostPath:
        return HostPath(self._home_dir)

    def getcwd(self) -> HostPath:
        return HostPath(self._cwd)
    ...
    async def iterdir(self, path: StrOrHostPath) -> AsyncGenerator[HostPath]:
        host_path = HostPath(path) if isinstance(path, str) else path
    ...
    async def glob(
        ...
        *,
        ...
        for entry in await self._sftp.glob(f"{real_path}/{pattern}"):
            yield HostPath(await self._sftp.realpath(str(entry)))
    ...
    async def exec(
        ...
        # - To make exec behave like other Host backends, we explicitly `cd` to our tracked
    ...
    async def unsafe_close(self) -> None:
        """Close the SSH connection. After that, SSHHost will be unusable."""
        if self._sftp:
            self._sftp.exit()
        if self._connection:
