...
class TestThinkingEffortMapping:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("medium", "medium"),
            ("high", "high"),
            # xhigh is a valid Pythinker Core level and a valid OpenAI level - round-trip.
            ("xhigh", "xhigh"),
        ],
    )
...
class TestOpenAIStreamingErrorPropagation:
    ...
    _convert_stream_response must convert it to the correct Pythinker Core error type.
    ...
    async def test_base_api_error_becomes_connection_error(self) -> None:
        ...
        must surface as Pythinker Core APIConnectionError."""
