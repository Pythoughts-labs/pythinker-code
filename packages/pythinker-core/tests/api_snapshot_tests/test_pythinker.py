"""Snapshot tests for Pythinker chat provider."""
...
async def test_pythinker_message_conversion():
    with respx.mock(base_url="https://api.pythinker-ai.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response("pythinker-ai"))
        )
        provider = Pythinker(model="pythinker-ai", api_key="test-key", stream=False)
...
async def test_pythinker_generation_kwargs():
    with respx.mock(base_url="https://api.pythinker-ai.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response())
        )
        provider = Pythinker(
            model="pythinker-ai", api_key="test-key", stream=False
...
async def test_pythinker_with_thinking():
    with respx.mock(base_url="https://api.pythinker-ai.ai") as mock:
        mock.post("/v1/chat/completions").mock(
            return_value=Response(200, json=make_chat_completion_response())
        )
...
async def test_pythinker_with_extra_body_thinking_deep_merge():
    ...
    with respx.mock(base_url="https://api.pythinker-ai.ai") as mock:
        ...
        provider = (
            Pythinker(model="pythinker-ai", api_key="test-key", stream=False)
            .with_thinking("high")
...
async def test_pythinker_with_extra_body_thinking_empty_dict_is_noop():
...
async def test_pythinker_with_extra_body_thinking_starts_from_empty_dict():
...
async def test_pythinker_with_extra_body_non_thinking_key_shallow_merge():
