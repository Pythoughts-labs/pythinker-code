try:
    ...
    raise ModuleNotFoundError(
        ...
        'Install with `pip install "pythinker-core[contrib]"`.'
    ) from exc
...
from pythinker_core.tooling import Tool as PythinkerCoreTool
...
class GoogleGenAI:
    ...
    async def generate(
        self,
        system_prompt: str,
        tools: Sequence[PythinkerCoreTool],
...
def tool_to_google_genai(tool: PythinkerCoreTool) -> Tool:
    """Convert a Pythinker Core tool to GoogleGenAI tool format."""
...
def _convert_error(error: Exception) -> ChatProviderError:
    """Convert a GoogleGenAI error to a Pythinker Core chat provider error."""
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        from pythinker_core.tooling import CallableTool2, ToolOk
        ...
        result = await pythinker_core.step(chat, system_prompt, toolset, history)
