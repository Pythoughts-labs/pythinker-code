...
from pythinker_core.utils.jsonschema import JsonDict, ensure_property_types

if TYPE_CHECKING:

    def type_check(pythinker: "Pythinker"):
        _: ChatProvider = pythinker
        _: RetryableChatProvider = pythinker


class ThinkingConfig(TypedDict, total=False):
    type: Literal["enabled", "disabled"]
    keep: Any
    """Pythinker AI-specific ``thinking.keep`` switch for preserved thinking.
...
class Pythinker:
    """
    A chat provider that uses the Pythinker API.

    >>> chat_provider = Pythinker(model="pythinker-ai", api_key="sk-1234567890")
    >>> chat_provider.name
    'pythinker'
    ...
    class GenerationKwargs(TypedDict, total=False):
        """
        See https://platform.pythinker-ai.ai/docs/api/chat#request-body.
        """
    ...
    def __init__(
        ...
        **client_kwargs: Any,
        ...
        if api_key is None:
            api_key = os.getenv("PYTHINKER_API_KEY")
        if api_key is None:
            raise ChatProviderError(
                "The api_key client option or the PYTHINKER_API_KEY environment variable is not set"
            )
        if base_url is None:
            base_url = os.getenv("PYTHINKER_BASE_URL", "https://api.pythinker-ai.ai/v1")
        ...
        self._generation_kwargs: Pythinker.GenerationKwargs = {}
    ...
    async def generate(
        ...
        generation_kwargs: dict[str, Any] = {
            # default pythinker generation kwargs
            "max_tokens": 32000,
        }
        ...
        try:
            ...
            return PythinkerStreamedMessage(response)
    ...
    def with_thinking(self, effort: ThinkingEffort) -> Self:
        match effort:
            ...
            case "high" | "xhigh" | "max":
                # Pythinker's API caps at "high"; xhigh/max are Anthropic-specific.
    ...
    @property
    def files(self) -> "PythinkerFiles":
        return PythinkerFiles(self.client)


class PythinkerFiles:
    def __init__(self, client: AsyncOpenAI) -> None:
        self._client = client

    async def upload_video(self, *, data: bytes, mime_type: str) -> VideoURLPart:
        """Upload a video to Pythinker files API and return a video URL content part."""
    ...
    async def _upload_file(
        self, *, data: bytes, mime_type: str, purpose: "PythinkerFilePurpose"
    ) -> str:
        ...
        try:
            response: PythinkerFileObject = await self._client.post(
                "/files",
                cast_to=PythinkerFileObject,
...
class PythinkerFileObject(BaseModel):
    id: str


type PythinkerFilePurpose = Literal["video", "image"]
...
def _convert_message(message: Message) -> ChatCompletionMessageParam:
    ...
    if (
        ...
        # `content`, but the Pythinker-for-Coding compat layer rejects a content
...
def _convert_tool(tool: Tool) -> ChatCompletionToolParam:
    ...
    # Pythinker AI's API rejects parameter schemas whose nested properties omit
    # `type` (e.g. enum-only properties exposed by some MCP servers). Patch
    # the schema locally so such tools keep working against Pythinker without
...
class PythinkerStreamedMessage:
    """The streamed message of the Pythinker chat provider."""
    ...
    def usage(self) -> TokenUsage | None:
        if self._usage:
            ...
            if hasattr(self._usage, "cached_tokens"):
                # https://platform.pythinker-ai.cn/docs/api/chat#%E8%BF%94%E5%9B%9E%E5%86%85%E5%AE%B9
                # TODO: delete this when Pythinker AI API becomes compatible with OpenAI API
...
if __name__ == "__main__":
    ...
    async def _dev_main():
        chat = Pythinker(model="pythinker-ai", stream=False)
        system_prompt = ""
        history = [
