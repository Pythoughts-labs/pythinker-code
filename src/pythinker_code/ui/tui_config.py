...
``pythinker`` worklog rendering path and the ``card`` style. Keeps the
flag-resolution logic in one place so the migration's escape hatch can be
removed cleanly once the card style stabilizes.

Card style is the default; set ``PYTHINKER_TUI_STYLE=pythinker`` or
``tui.style = "pythinker"`` to use legacy rendering.
"""
