...
async def _collect_entries(
    dir_path: HostPath, max_width: int
...
async def list_directory(work_dir: HostPath) -> str:
    ...
    ``PYTHINKER_WORK_DIR_LS``) and to show top-level directory contents in tools.
...
def is_within_directory(path: HostPath, directory: HostPath) -> bool:
    """
    Check whether *path* is contained within *directory* using pure path semantics.
    Both arguments should already be canonicalized (e.g. via HostPath.canonical()).
    """
