...
async def _find_session_in_work_dir(work_dir: HostPath, session_id: str) -> Session | None:
    from pythinker_code.session import Session

    return await Session.find(work_dir, session_id)


async def _load_previous_session(work_dir: HostPath) -> Session | None:
    from pythinker_code.session import Session

    return await Session.continue_(work_dir)


def _resolve_work_dir(ctx: typer.Context) -> HostPath:
    root_ctx = ctx.find_root()
    local_work_dir = root_ctx.params.get("local_work_dir")
    if local_work_dir is None:
        return HostPath.cwd()
    return HostPath.unsafe_from_local_path(local_work_dir)


def _find_session_by_id(session_id: str, *, work_dir: HostPath | None = None) -> Path | None:
...
def _collect_recent_log_files(session_dir: Path) -> list[Path]:
    ...
    for f in log_dir.iterdir():
        if not f.is_file() or not f.name.startswith("pythinker.") or not f.name.endswith(".log"):
            continue
        try:
            st = f.stat()
            mtime = st.st_mtime
...
def _build_manifest(session_id: str, session_dir: Path) -> dict[str, str | None]:
    ...
    try:
        ...
        manifest["pythinker_code_version"] = get_version()
