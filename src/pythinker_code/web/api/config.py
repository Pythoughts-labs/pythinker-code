...
class ConfigModel(LLMModel):
    ...
    name: str = Field(description="Model key in pythinker-code config (Config.models)")
...
def _build_global_config() -> GlobalConfig:
    """Build GlobalConfig from pythinker-code config."""
    config = load_config()
...
def _get_runner(req: Request) -> PythinkerCLIRunner:
    """Get PythinkerCLIRunner from FastAPI app state."""
...
@router.get("/", summary="Get global (pythinker-code) config snapshot")
async def get_global_config() -> GlobalConfig:
    """Get global (pythinker-code) config snapshot."""
    return _build_global_config()


@router.patch("/", summary="Update global (pythinker-code) default model/thinking")
async def update_global_config(
    request: UpdateGlobalConfigRequest,
    http_request: Request,
    runner: PythinkerCLIRunner = Depends(_get_runner),
) -> UpdateGlobalConfigResponse:
    """Update global (pythinker-code) default model/thinking."""
