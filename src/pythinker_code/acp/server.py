...
from pythinker_code.acp.host import ACPHost
from pythinker_code.acp.mcp import acp_mcp_servers_to_mcp_config
from pythinker_code.acp.session import ACPSession
from pythinker_code.acp.tools import replace_tools
from pythinker_code.acp.types import ACPContentBlock, MCPServer
from pythinker_code.acp.version import ACPVersionSpec, negotiate_version
from pythinker_code.app import PythinkerCLI
from pythinker_code.auth.oauth import PYTHINKER_CODE_OAUTH_KEY, load_tokens
from pythinker_code.config import Config, LLMModel, OAuthRef, load_config, save_config
from pythinker_code.constant import NAME, VERSION
from pythinker_code.llm import create_llm, derive_model_capabilities
...
class ACPServer:
    ...
    @staticmethod
    def _check_oauth_ref_usable(ref: OAuthRef) -> str | None:
    ...
    @classmethod
    def _check_token_usable(cls) -> str | None:
        ref = OAuthRef(storage="file", key=PYTHINKER_CODE_OAUTH_KEY)
        return cls._check_oauth_ref_usable(ref)

    @classmethod
    def _check_config_auth(cls, config: Config) -> str | None:
        """Validate credentials for the active model provider, if one is configured."""
        if not config.default_model:
            return cls._check_token_usable()
        model = config.models.get(config.default_model)
        if model is None:
            return f"default model {config.default_model!r} not found"
        provider = config.providers.get(model.provider)
        if provider is None:
            return f"provider {model.provider!r} not found"
        if provider.type in {"_echo", "_scripted_echo", "_chaos"}:
            return None
        if provider.oauth is not None:
            return cls._check_oauth_ref_usable(provider.oauth)
        if provider.api_key.get_secret_value():
            return None
        return f"provider {model.provider!r} has no API key configured"

    def _check_auth(self, config: Config | None = None) -> None:
        """Check if Pythinker authentication is complete. Raise AUTH_REQUIRED if not."""
        reason = (
    ...
    async def new_session(
        ...
        **kwargs: Any,
        ...
        session = await Session.create(HostPath.unsafe_from_local_path(Path(cwd)))

        mcp_config = acp_mcp_servers_to_mcp_config(mcp_servers or [])
        cli_instance = await PythinkerCLI.create(
            session,
            mcp_configs=[mcp_config],
            ui_mode="acp",
        )
        config = cli_instance.soul.runtime.config
        acp_host = ACPHost(self.conn, session.id, self.client_capabilities)
        acp_session = ACPSession(session.id, cli_instance, self.conn, host=acp_host)
        ...
        if isinstance(cli_instance.soul.agent.toolset, PythinkerToolset):
            replace_tools(
    ...
    async def _setup_session(
        ...
        work_dir = HostPath.unsafe_from_local_path(Path(cwd))
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> acp.AuthenticateResponse | None:
        ...
        """
        if method_id == "login":
            reason = self._check_config_auth(load_config())
            if reason is None:
