...
TRACE_ENV = "PYTHINKER_TEST_TRACE"
WIRE_COMMAND_ENV = "PYTHINKER_E2E_WIRE_CMD"
DEFAULT_TIMEOUT = 5.0
...
def make_env(home_dir: Path) -> dict[str, str]:
    ...
    env["PYTHINKER_SHARE_DIR"] = str(share_dir(home_dir))
    return env


def share_dir(home_dir: Path) -> Path:
    return home_dir / ".pythinker"
...
def _normalize_server_version(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        ...
        if value.get("name") == "Pythinker CLI" and "version" in value:
...
def base_command() -> list[str]:
    ...
    parts = shlex.split(override, posix=os.name != "nt") if override else ["uv", "run", "pythinker"]
