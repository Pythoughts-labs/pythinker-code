...
- `/export`: Export to the current working directory with an auto-generated filename (format: `pythinker-export-<first 8 chars of session ID>-<timestamp>.md`)
...
::: tip
Directories already within the working directory do not need to be added, as they are already accessible. You can also add directories at startup via the `--add-dir` option. See [`pythinker` command](./pythinker-command.md#working-directory) for details.
:::
