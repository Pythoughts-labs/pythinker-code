...
| `--config-file PATH` | Load configuration file (default `~/.pythinker/config.toml`) |
...
| Option | Description |
|--------|-------------|
| `--print` | Run in print mode (non-interactive), implicitly enables `--auto` |
| `--quiet` | Shortcut for `--print --output-format text --final-message-only` |
| `--acp` | Run in ACP server mode (deprecated, use `pythinker acp` instead) |
...
Default loads `~/.pythinker/mcp.json` (if exists). See [Model Context Protocol](../customization/mcp.md) for details.
...
When not specified, Pythinker Code automatically discovers user-level and project-level skills directories in priority order. See [Agent Skills](../customization/skills.md) for details.

## Subcommands

| Subcommand | Description |
|------------|-------------|
| [`pythinker login`](#pythinker-login) | Log in to your Pythinker account |
| [`pythinker logout`](#pythinker-logout) | Log out from your Pythinker account |
| [`pythinker info`](./pythinker-info.md) | Display version and protocol information |
| [`pythinker acp`](./pythinker-acp.md) | Start multi-session ACP server |
| [`pythinker mcp`](./pythinker-mcp.md) | Manage MCP server configuration |
...
See [Web UI](./pythinker-web.md) for details.
