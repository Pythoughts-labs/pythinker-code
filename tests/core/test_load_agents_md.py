...
from pythinker_code.soul.agent import _AGENTS_MD_MAX_BYTES, load_agents_md
...
async def test_found(temp_work_dir: HostPath):
...
async def test_not_found(temp_work_dir: HostPath):
...
async def test_lowercase(temp_work_dir: HostPath):
...
async def test_uppercase_over_lowercase(temp_work_dir: HostPath):
...
async def test_hierarchical_three_levels(temp_work_dir: HostPath):
...
async def test_no_git_fallback(temp_work_dir: HostPath):
...
async def test_empty_file_skipped(temp_work_dir: HostPath):
...
async def test_max_bytes_leaf_prioritised(temp_work_dir: HostPath):
