...
from pythinker_code.wire.file import WireMessageRecord
from pythinker_code.wire.serde import deserialize_wire_message, serialize_wire_message
...
async def test_wire_message_serde():
    ...
    msg = PlanDisplay(
        content="## Plan\n\n1. Step one\n2. Step two",
        file_path="/Users/test/.pythinker/plans/iron-man-spider-man.md",
    )
    assert serialize_wire_message(msg) == snapshot(
        {
            "type": "PlanDisplay",
...
def test_wire_message_type_alias():
    import pythinker_code.wire.types
