...
def _roots_of(scoped: list[ScopedSkillsRoot]) -> list[HostPath]:
...
def _as_user(dirs: list[HostPath]) -> list[ScopedSkillsRoot]:
    """Wrap a list of HostPath into ScopedSkillsRoot with scope='user'. Used by
    tests that only exercise discovery ordering, not scope semantics.
    """
    return [ScopedSkillsRoot(root=d, scope="user") for d in dirs]


def _scoped(**by_scope: list[HostPath]) -> list[ScopedSkillsRoot]:
...
""",
    ...
    root_path = HostPath.unsafe_from_local_path(root)
    skills = await discover_skills(root_path, scope="user")
    base_dir = HostPath.unsafe_from_local_path(Path("/path/to"))
    for skill in skills:
    ...
    assert skills == snapshot(
        [
            Skill(
                ...
                description="Alpha description",
                type="standard",
                dir=HostPath.unsafe_from_local_path(Path("/path/to/alpha")),
                skill_md_file=HostPath.unsafe_from_local_path(Path("/path/to/alpha/SKILL.md")),
                flow=None,
                scope="user",
            ),
            Skill(
                ...
                type="standard",
                dir=HostPath.unsafe_from_local_path(Path("/path/to/beta")),
                skill_md_file=HostPath.unsafe_from_local_path(Path("/path/to/beta/SKILL.md")),
                flow=None,
                scope="user",
            ),
        ]
...
""",
    ...
    skills = await discover_skills(HostPath.unsafe_from_local_path(root), scope="user")
...
""",
    ...
    skills = await discover_skills_from_roots(
        [
            ScopedSkillsRoot(root=HostPath.unsafe_from_local_path(system_dir), scope="builtin"),
            ScopedSkillsRoot(root=HostPath.unsafe_from_local_path(user_dir), scope="user"),
        ]
    )
    base_dir = HostPath.unsafe_from_local_path(Path("/path/to"))
    for skill in skills:
    ...
    assert skills == snapshot(
        [
            Skill(
                ...
                description="System version",
                type="standard",
                dir=HostPath.unsafe_from_local_path(Path("/path/to/system/shared")),
                skill_md_file=HostPath.unsafe_from_local_path(
...
async def test_resolve_skills_roots_uses_layers(monkeypatch, tmp_path):
    ...
    Fix 7 (scope priority): higher-priority roots come first so
    ...
    roots = _roots_of(await resolve_skills_roots(HostPath.unsafe_from_local_path(work_dir)))

    # Project (most specific) first, then user, then builtin (least specific).
    assert roots == [
        HostPath.unsafe_from_local_path(project_dir),
        HostPath.unsafe_from_local_path(user_dir),
...
async def test_resolve_skills_roots_skills_dirs_override_discovery(tmp_path, monkeypatch):
    ...
    roots = _roots_of(
        await resolve_skills_roots(
            ...
            skills_dirs=[
                HostPath.unsafe_from_local_path(extra_a),
                HostPath.unsafe_from_local_path(extra_b),
            ],
        )
    )
...
async def test_resolve_skills_roots_empty_skills_dirs(tmp_path, monkeypatch):
    ...
    roots_none = await resolve_skills_roots(
        HostPath.unsafe_from_local_path(tmp_path),
        skills_dirs=None,
    )
...
async def test_discover_skills_from_roots_first_wins(tmp_path):
    ...
    skills = await discover_skills_from_roots(
        _as_user(
            [
                HostPath.unsafe_from_local_path(tmp_path / "root_a"),
                HostPath.unsafe_from_local_path(tmp_path / "root_b"),
            ]
        )
    )
...
async def test_find_user_skills_dirs_empty_generic_does_not_shadow_brand(monkeypatch, tmp_path):
    """Core bug: empty ~/.config/agents/skills should NOT hide ~/.pythinker/skills."""
    home_dir = tmp_path / "home"
    generic_dir = home_dir / ".config" / "agents" / "skills"
    generic_dir.mkdir(parents=True)  # exists but empty

    brand_dir = home_dir / ".pythinker" / "skills"
    ...
    assert len(dirs) == 2
    assert dirs[0] == HostPath.unsafe_from_local_path(brand_dir)
    assert dirs[1] == HostPath.unsafe_from_local_path(generic_dir)
...
async def test_find_user_skills_dirs_only_brand(monkeypatch, tmp_path):
    ...
    dirs = await find_user_skills_dirs()
    assert dirs == [HostPath.unsafe_from_local_path(brand_dir)]
...
async def test_find_user_skills_dirs_only_generic(monkeypatch, tmp_path):
    ...
    dirs = await find_user_skills_dirs()
    assert dirs == [HostPath.unsafe_from_local_path(generic_dir)]
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_brand_group_prefers_pythinker_over_claude(
    monkeypatch, tmp_path
):
    """Brand group: ~/.pythinker/skills takes priority over ~/.claude/skills."""
    home_dir = tmp_path / "home"
    pythinker_dir = home_dir / ".pythinker" / "skills"
    pythinker_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    claude_dir.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home_dir)

    dirs = await find_user_skills_dirs()
    # Only pythinker should be selected (first existing in brand group)
    assert HostPath.unsafe_from_local_path(pythinker_dir) in dirs
    assert HostPath.unsafe_from_local_path(claude_dir) not in dirs
...
async def test_find_project_skills_dirs_merge(tmp_path):
    ...
    generic_dir.mkdir(parents=True)
    brand_dir = work_dir / ".pythinker" / "skills"
    brand_dir.mkdir(parents=True)

    dirs = await find_project_skills_dirs(HostPath.unsafe_from_local_path(work_dir))
    assert len(dirs) == 2
    assert dirs[0] == HostPath.unsafe_from_local_path(brand_dir)
    assert dirs[1] == HostPath.unsafe_from_local_path(generic_dir)
...
@pytest.mark.asyncio
async def test_find_project_skills_dirs_brand_prefers_pythinker(tmp_path):
    """Project layer brand group: .pythinker/skills wins over .claude/skills."""
    work_dir = tmp_path / "project"
    pythinker_dir = work_dir / ".pythinker" / "skills"
    pythinker_dir.mkdir(parents=True)
    ...
    dirs = await find_project_skills_dirs(HostPath.unsafe_from_local_path(work_dir))
    assert len(dirs) == 1
    assert dirs[0] == HostPath.unsafe_from_local_path(pythinker_dir)
...
async def test_resolve_skills_roots_merges_user_and_project(monkeypatch, tmp_path):
    ...
    user_brand = home_dir / ".pythinker" / "skills"
    ...
    proj_brand = work_dir / ".pythinker" / "skills"
    proj_brand.mkdir(parents=True)

    roots = _roots_of(await resolve_skills_roots(HostPath.unsafe_from_local_path(work_dir)))
    assert roots == [
        HostPath.unsafe_from_local_path(proj_brand),
        HostPath.unsafe_from_local_path(proj_generic),
        HostPath.unsafe_from_local_path(user_brand),
        HostPath.unsafe_from_local_path(user_generic),
        HostPath.unsafe_from_local_path(get_builtin_skills_dir()),
    ]
...
async def test_empty_generic_brand_skills_visible_end_to_end(monkeypatch, tmp_path):
    ...
    work_dir = tmp_path / "project"
    scoped = await resolve_skills_roots(HostPath.unsafe_from_local_path(work_dir))
...
async def test_find_user_skills_dirs_generic_group_prefers_config_over_agents(
    ...
    dirs = await find_user_skills_dirs()
    assert HostPath.unsafe_from_local_path(config_dir) in dirs
    assert HostPath.unsafe_from_local_path(agents_dir) not in dirs
...
@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_pythinker_and_claude(monkeypatch, tmp_path):
    """merge_brands=True: pythinker + claude both exist → both returned, pythinker first."""
    home_dir = tmp_path / "home"
    pythinker_dir = home_dir / ".pythinker" / "skills"
    pythinker_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    ...
    assert HostPath.unsafe_from_local_path(pythinker_dir) in dirs
    assert HostPath.unsafe_from_local_path(claude_dir) in dirs
    # pythinker before claude
    pythinker_idx = dirs.index(HostPath.unsafe_from_local_path(pythinker_dir))
    claude_idx = dirs.index(HostPath.unsafe_from_local_path(claude_dir))
    assert pythinker_idx < claude_idx


@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_all_three(monkeypatch, tmp_path):
    """merge_brands=True: all three brand dirs → [pythinker, claude, codex]."""
    home_dir = tmp_path / "home"
    pythinker_dir = home_dir / ".pythinker" / "skills"
    pythinker_dir.mkdir(parents=True)
    claude_dir = home_dir / ".claude" / "skills"
    ...
    assert brand_dirs[0] == HostPath.unsafe_from_local_path(pythinker_dir)
    assert brand_dirs[1] == HostPath.unsafe_from_local_path(claude_dir)
    assert brand_dirs[2] == HostPath.unsafe_from_local_path(codex_dir)
...
async def test_find_user_skills_dirs_merge_brands_only_claude(monkeypatch, tmp_path):
    ...
    dirs = await find_user_skills_dirs(merge_brands=True)
    assert dirs == [HostPath.unsafe_from_local_path(claude_dir)]


@pytest.mark.asyncio
async def test_find_user_skills_dirs_merge_brands_same_skill_pythinker_wins(monkeypatch, tmp_path):
    """merge_brands=True + same skill name → pythinker version wins via discover."""
    home_dir = tmp_path / "home"
    pythinker_dir = home_dir / ".pythinker" / "skills"
    pythinker_dir.mkdir(parents=True)
    _write_skill(
        pythinker_dir / "deploy",
        "---\nname: deploy\ndescription: pythinker deploy\n---\n",
    )
    claude_dir = home_dir / ".claude" / "skills"
    claude_dir.mkdir(parents=True)
    _write_skill(
    ...
    skills = await discover_skills_from_roots(_as_user(dirs))
    assert len(skills) == 1
    assert skills[0].description == "pythinker deploy"
...
async def test_find_project_skills_dirs_merge_brands(tmp_path):
    ...
    dirs = await find_project_skills_dirs(
        HostPath.unsafe_from_local_path(work_dir), merge_brands=True
    )
    assert HostPath.unsafe_from_local_path(pythinker_dir) in dirs
    assert HostPath.unsafe_from_local_path(claude_dir) in dirs
...
def test_get_builtin_skills_dir_frozen_env(monkeypatch, tmp_path):
    ...
    assert result == fake_meipass / "pythinker_code" / "skills"
...
def test_get_builtin_skills_dir_normal_env():
    ...
    assert result.parent.name == "pythinker_code"
...
async def test_resolve_skills_roots_passes_merge_brands(monkeypatch, tmp_path):
    ...
    # Without merge_brands: only pythinker
    roots_default = _roots_of(await resolve_skills_roots(HostPath.unsafe_from_local_path(work_dir)))
    assert HostPath.unsafe_from_local_path(pythinker_dir) in roots_default
    assert HostPath.unsafe_from_local_path(claude_dir) not in roots_default
    ...
    assert HostPath.unsafe_from_local_path(pythinker_dir) in roots_merged
    assert HostPath.unsafe_from_local_path(claude_dir) in roots_merged
...
async def test_resolve_skills_roots_extra_skill_dirs_append(monkeypatch, tmp_path):
    ...
    assert HostPath.unsafe_from_local_path(extra) in roots
...
async def test_resolve_skills_roots_extra_skill_dirs_expand_tilde(monkeypatch, tmp_path):
    ...
    assert HostPath.unsafe_from_local_path(extra_under_home) in roots
...
async def test_resolve_skills_roots_extra_skill_dirs_relative_to_project_root(
    ...
    assert HostPath.unsafe_from_local_path(extra_at_root) in roots
    # Must not have resolved to <work_dir>/my-dir (which doesn't exist)
    assert HostPath.unsafe_from_local_path(work_dir / "my-dir") not in roots
...
async def test_resolve_skills_roots_extra_skill_dirs_absolute(monkeypatch, tmp_path):
    ...
    assert HostPath.unsafe_from_local_path(abs_extra) in roots
...
async def test_resolve_skills_roots_extra_skill_dirs_missing_path_skipped(monkeypatch, tmp_path):
    ...
    assert HostPath.unsafe_from_local_path(real) in roots
    # Missing paths did not make it into the final roots list
    assert HostPath.unsafe_from_local_path(tmp_path / "nowhere") not in roots
...
async def test_resolve_skills_roots_extra_skill_dirs_dedup(monkeypatch, tmp_path):
    ...
    real_kp = HostPath.unsafe_from_local_path(real)
...
async def test_resolve_skills_roots_extra_skill_dirs_cli_override_still_wins(monkeypatch, tmp_path):
    ...
    roots = _roots_of(
        await resolve_skills_roots(
            HostPath.unsafe_from_local_path(work_dir),
            skills_dirs=[HostPath.unsafe_from_local_path(cli_dir)],
            extra_skill_dirs=[str(extra_dir)],
        )
    )

    assert HostPath.unsafe_from_local_path(cli_dir) in roots
    assert HostPath.unsafe_from_local_path(extra_dir) in roots
...
async def test_project_scope_skill_overrides_builtin_with_same_name(monkeypatch, tmp_path):
    ...
    import pythinker_code.skill as skill_mod
    ...
    work_dir = tmp_path / "project"
    proj_dir = work_dir / ".pythinker" / "skills"
...
async def test_user_scope_skill_overrides_builtin_with_same_name(monkeypatch, tmp_path):
    ...
    home_dir = tmp_path / "home"
    user_dir = home_dir / ".pythinker" / "skills"
    user_dir.mkdir(parents=True)
    (user_dir / "foo").mkdir()
...
async def test_resolve_skills_roots_extra_skill_dirs_symlink_dedup(monkeypatch, tmp_path):
    ...
    ``HostPath.canonical()`` does not walk symlinks, so this relies on
...
async def test_discover_skills_permission_denied_returns_empty(monkeypatch, tmp_path):
    ...
    root.mkdir()
    root_kp = HostPath.unsafe_from_local_path(root)
...
def test_skill_scope_is_required_field(tmp_path):
    ...
    Contract test for the "scope-is-required" decision: the default value was
    ...
    with pytest.raises(pydantic.ValidationError):
        Skill(  # type: ignore[call-arg]
            ...
            description="x",
            type="standard",
            dir=HostPath.unsafe_from_local_path(tmp_path),
            skill_md_file=HostPath.unsafe_from_local_path(tmp_path / "SKILL.md"),
            flow=None,
        )
...
async def test_extra_skill_dirs_dedup_with_auto_discovery_overlap(monkeypatch, tmp_path):
    ...
    Scenario: user puts ``~/.pythinker/skills`` (already auto-discovered as the
    ...
    user_brand_kp = HostPath.unsafe_from_local_path(user_brand.resolve())
...
# the work directory. Launching pythinker-code from a subdirectory of a monorepo
...
async def test_find_project_skills_dirs_walks_up_to_git_root(tmp_path):
    ...
    defined at the repo root — otherwise running pythinker-code inside a monorepo
    ...
    dirs = await find_project_skills_dirs(HostPath.unsafe_from_local_path(nested))

    # The repo-root .claude/skills is discovered, not some phantom dir under
    # the nested work dir.
    assert dirs == [HostPath.unsafe_from_local_path(repo_claude_skills)]
...
async def test_find_project_skills_dirs_cwd_is_project_root_still_works(tmp_path):
    ...
    dirs = await find_project_skills_dirs(HostPath.unsafe_from_local_path(repo))

    assert dirs == [HostPath.unsafe_from_local_path(claude_skills)]
...
# ~/.pythinker/skills and ~/.claude/skills after migrating from Claude Code)
# previously lost visibility of everything in .claude/skills because the
# default "first brand wins" behaviour would stop at .pythinker/skills. Flipping
...
async def test_default_config_effectively_merges_user_brand_skill_dirs(monkeypatch, tmp_path):
    ...
    both ``~/.pythinker/skills`` and ``~/.claude/skills`` are surfaced. If the
    ...
    assert HostPath.unsafe_from_local_path(pythinker_dir) in roots
    assert HostPath.unsafe_from_local_path(claude_dir) in roots
