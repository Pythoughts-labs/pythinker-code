...
from pythinker_code.approval_runtime import ApprovalSource, get_current_approval_source_or_none
from pythinker_code.background import TaskRuntime, TaskSpec
from pythinker_code.llm import LLM
from pythinker_code.notifications import NotificationEvent
from pythinker_code.soul import RunCancelled, StatusSnapshot, _current_wire, run_soul
...
from pythinker_code.wire.types import ApprovalRequest, ApprovalResponse, Notification
...
def _make_soul(runtime: Runtime, tmp_path: Path) -> tuple[PythinkerSoul, Context]:
    llm = LLM(
...
@pytest.mark.asyncio
async def test_pythinkersoul_appends_notification_message(runtime: Runtime, tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_pythinker_code_run_yields_root_hub_approvals(runtime: Runtime) -> None:
    ...
    cli = PythinkerCLI(_ApprovalOnlySoul(runtime), runtime, {})  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_pythinker_code_run_bridges_approval_resolution_back_to_runtime(
    runtime: Runtime,
) -> None:
    ...
    cli = PythinkerCLI(soul, runtime, {})  # type: ignore[arg-type]
...
@pytest.mark.asyncio
async def test_pythinker_code_run_cancels_abandoned_approval_stream(
    runtime: Runtime,
    tmp_path: Path,
    ...
    monkeypatch.setattr(PythinkerSoul, "_turn", fake_turn)
...
@pytest.mark.asyncio
async def test_pythinker_code_run_propagates_external_cancel_event(
    runtime: Runtime,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_pythinker_code_run_replays_pending_approvals_from_previous_turn(
    runtime: Runtime,
) -> None:
    assert runtime.approval_runtime is not None
