"""Tests for --plan flag and default_plan_mode config in PythinkerCLI.create()."""
...
# ---------------------------------------------------------------------------
# Helpers — lightweight fakes for PythinkerCLI.create() dependencies
# ---------------------------------------------------------------------------


def _patch_create_deps(monkeypatch, *, session_plan_mode: bool = False):
    """Patch heavy dependencies so PythinkerCLI.create() runs without I/O.

    Returns a FakeSoul class whose instances record plan-mode interactions.
    """

    class FakeSoul:
        """Tracks plan-mode calls made by PythinkerCLI.create()."""
    ...
    async def fake_runtime_create(config, _oauth, _llm, session, yolo, **kwargs):
        ...
        return SimpleNamespace(
            ...
            approval=SimpleNamespace(
                is_yolo=lambda: yolo,
                is_auto=lambda: kwargs.get("auto", False) or kwargs.get("runtime_auto", False),
            ),
    ...
    monkeypatch.setattr(app_module, "augment_provider_with_env_vars", lambda p, m, **kw: {})
    ...
    monkeypatch.setattr(app_module, "PythinkerSoul", FakeSoul)
...
class TestPlanFlagNewSession:
    ...
    async def test_plan_flag_activates_plan_mode(self, session, config, monkeypatch):
        ...
        await PythinkerCLI.create(session, config=config, plan_mode=True, resumed=False)
    ...
    async def test_config_default_plan_mode_activates(self, session, config, monkeypatch):
        ...
        await PythinkerCLI.create(session, config=config, plan_mode=False, resumed=False)
...
class TestPlanFlagResumedSession:
    ...
    async def test_config_not_applied_on_resumed_session(self, session, config, monkeypatch):
        ...
        await PythinkerCLI.create(session, config=config, plan_mode=False, resumed=True)
    ...
    async def test_plan_flag_activates_on_resumed_session_without_plan(
        ...
        await PythinkerCLI.create(session, config=config, plan_mode=True, resumed=True)
...
class TestPlanFlagPriority:
    ...
    async def test_default_is_false(self, session, config, monkeypatch):
        ...
        await PythinkerCLI.create(session, config=config, resumed=False)
    ...
    async def test_plan_and_yolo_coexist(self, session, config, monkeypatch):
        ...
        await PythinkerCLI.create(session, config=config, plan_mode=True, yolo=True, resumed=False)
    ...
    @pytest.mark.asyncio
    async def test_runtime_auto_passed_separately_from_auto(self, session, config, monkeypatch):
        """Print-mode auto overlay should be passed separately from explicit auto."""
        _, runtime_create_calls = _patch_create_deps(monkeypatch)

        await PythinkerCLI.create(session, config=config, runtime_auto=True, resumed=False)

        assert runtime_create_calls[0]["auto"] is False
        assert runtime_create_calls[0]["runtime_auto"] is True


class TestSchedulePlanActivationReminder:
    """Unit tests for PythinkerSoul.schedule_plan_activation_reminder()."""
    ...
    def test_schedules_when_in_plan_mode(self, runtime, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("pythinker_code.tools.plan.heroes.PLANS_DIR", tmp_path)
        ...
        soul = PythinkerSoul(agent, context=Context(file_backend=tmp_path / "ctx.jsonl"))
...
class TestWebWorkerResumedDetection:
    ...
    async def test_new_session_without_state_file_is_not_resumed(self, tmp_path):
        ...
        with (
            patch("pythinker_code.web.runner.worker.load_session_by_id", return_value=fake_joint),
            patch(
                "pythinker_code.web.runner.worker.get_global_mcp_config_file",
                return_value=tmp_path / "no-mcp.json",
            ),
            patch.object(PythinkerCLI, "create", side_effect=spy_create),
