...
def _is_permissions_state_injection(message: Message) -> bool:
    return (
        message.role == "user"
        and is_system_reminder_message(message)
        and "Permissions state:" in message.extract_text(" ")
    )
...
async def test_consume_pending_steers_appends_history_before_emitting_wire_event(
    ...
    monkeypatch.setattr(pythinkersoul_module, "wire_send", fake_wire_send)
...
async def test_agent_loop_injects_steer_between_completed_steps(
    ...
    async def fake_step():
        ...
        return pythinkersoul_module.StepOutcome(
