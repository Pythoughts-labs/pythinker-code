...
@pytest.mark.asyncio
async def test_pythinker_code_create_reports_startup_phases(session, config, monkeypatch) -> None:
    phases: list[str] = []
    ...
    assert isinstance(cli, PythinkerCLI)
    assert phases == [
...
@pytest.mark.asyncio
async def test_pythinker_code_create_cleans_stale_running_foreground_subagents(
    session, config, monkeypatch
) -> None:
    ...
    await PythinkerCLI.create(session, config=config)
