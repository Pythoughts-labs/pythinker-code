...
def _make_config() -> Config:
    ...
    model = LLMModel(
        provider="managed:pythinker-code", model="test-model", max_context_size=100_000
    )
    return Config(
        default_model="managed:pythinker-code/test-model",
        providers={"managed:pythinker-code": provider},
        models={"managed:pythinker-code/test-model": model},
        services=Services(),
    )


def _make_manager(token: OAuthToken | None = None) -> OAuthManager:
    with patch("pythinker_code.auth.oauth.load_tokens", return_value=token):
...
async def test_refresh_token_does_not_retry_on_unauthorized():
    ...
    with (
        patch("pythinker_code.auth.oauth.new_client_session", return_value=FakeSession()),
...
async def test_ensure_fresh_force_bypasses_threshold():
    ...
    with (
        ...
        patch("pythinker_code.auth.oauth.refresh_token", mock_refresh),
...
async def test_ensure_fresh_force_raises_on_unauthorized():
    ...
    with (
        ...
        patch("pythinker_code.auth.oauth.asyncio.sleep", new=AsyncMock()),
...
async def test_unauthorized_must_not_delete_credentials_file(tmp_path, monkeypatch):
    ...
    _save_to_file("oauth/pythinker-code", _make_token(refresh="R1", expires_in=100))
    cred = tmp_path / "credentials" / "pythinker-code.json"
    assert cred.exists()
    ...
    assert manager._access_tokens.get("oauth/pythinker-code") is None, (
...
async def test_rejected_refresh_token_cooldown_skips_background_retry(tmp_path, monkeypatch):
    ...
    with (
        patch("pythinker_code.auth.oauth.refresh_token", refresh),
        patch("pythinker_code.auth.oauth.asyncio.sleep", new=AsyncMock()),
    ):
...
async def test_rejected_tombstone_cleared_when_concurrent_instance_rotated(tmp_path, monkeypatch):
    """If another pythinker-code instance legitimately rotates the refresh token
    ...
    assert _REJECTED_REFRESH_TOKENS.get("oauth/pythinker-code") is not None

    # Step 2: simulate a concurrent instance writing a fresh token (R2) to disk
    _save_to_file(
        "oauth/pythinker-code",
    ...
    assert _REJECTED_REFRESH_TOKENS.get("oauth/pythinker-code") is None, (
        "tombstone must be cleared once the on-disk refresh_token no longer matches"
    )
    assert manager._access_tokens.get("oauth/pythinker-code") == "new-access", (
...
async def test_ensure_fresh_non_force_swallows_errors():
    ...
    with (
        patch("pythinker_code.auth.oauth.load_tokens", return_value=token),
        patch("pythinker_code.auth.oauth.refresh_token", AsyncMock(side_effect=OAuthError("fail"))),
    ):
