...
def _run_pythinker(args: list[str], *, share_dir: Path) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    ...
    cmd = [sys.executable, "-m", "pythinker_code.cli", *args]
...
def test_config_option_requires_argument_is_reported(tmp_path: Path) -> None:
    ...
    result = _run_pythinker(["--config"], share_dir=share_dir)
...
def test_config_option_help_value_is_reported(tmp_path: Path) -> None:
    share_dir = tmp_path / "share"
    result = _run_pythinker(["--config", "--help"], share_dir=share_dir)
    assert result.returncode == snapshot(2)
    assert result.stdout == snapshot("")
    normalized = _normalize_cli_error_output(result.stderr)
    assert normalized.startswith(
        """\
Usage: python -m pythinker_code.cli [OPTIONS] COMMAND [ARGS]...
Try 'python -m pythinker_code.cli -h' for help.
Error:
"""
    )
    assert (
...
def test_invalid_config_toml_is_reported(tmp_path: Path) -> None:
    ...
    result = _run_pythinker(
        ["--print", "--yolo", "--prompt", "hello", "--config-file", str(config_path)],
        share_dir=share_dir,
    )

    log_path = share_dir / "logs" / "pythinker.log"
...
def test_session_and_continue_conflict_is_reported(tmp_path: Path) -> None:
    share_dir = tmp_path / "share"
    result = _run_pythinker(["--session", "abc", "--continue"], share_dir=share_dir)
    assert result.returncode == snapshot(2)
    assert result.stdout == snapshot("")
    assert _normalize_cli_error_output(result.stderr) == snapshot(
        """\
...
def test_session_picker_with_print_mode_is_reported(tmp_path: Path) -> None:
    share_dir = tmp_path / "share"
    result = _run_pythinker(["--session", "--print", "--prompt", "hi"], share_dir=share_dir)
    assert result.returncode == snapshot(2)
    assert result.stdout == snapshot("")
    assert _normalize_cli_error_output(result.stderr) == snapshot(
        """\
...
def test_resume_alias_and_continue_conflict_is_reported(tmp_path: Path) -> None:
    share_dir = tmp_path / "share"
    result = _run_pythinker(["--resume", "abc", "--continue"], share_dir=share_dir)
    assert result.returncode == snapshot(2)
    assert result.stdout == snapshot("")
    assert _normalize_cli_error_output(result.stderr) == snapshot(
        """\
