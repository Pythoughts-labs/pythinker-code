...
def test_shell_running_prompt_preserves_unsubmitted_draft(tmp_path: Path) -> None:
    ...
    try:
        ...
        shell.read_until_contains("Bash sleep 1.5", after=first_turn_mark, timeout=15.0)
        time.sleep(0.3)
...
def test_shell_running_prompt_ignores_shift_tab_plan_toggle(tmp_path: Path) -> None:
    ...
    try:
        ...
        shell.send_line("start long turn")
        shell.read_until_contains("Bash sleep 1.5", after=turn_mark, timeout=15.0)
        ...
        json.dumps(
            {
                ...
                "providers": {
                    "root_provider": {
                        ...
                        "base_url": "",
                        "api_key": "",
                        "env": {"PYTHINKER_SCRIPTED_ECHO_SCRIPTS": str(root_scripts_path)},
                    },
                    "sub_provider": {
                        "type": "_scripted_echo",
                        "base_url": "",
                        "api_key": "",
                        "env": {"PYTHINKER_SCRIPTED_ECHO_SCRIPTS": str(sub_scripts_path)},
                    },
                },
            }
        ),
...
def test_shell_approval_reject_and_recover(tmp_path: Path) -> None:
    ...
    try:
        ...
        shell.send_key("3")
        # Wait for the tool call to be fully processed before looking for the
        # prompt, to avoid matching ✨ from a mid-turn redraw.
        shell.read_until_contains(
