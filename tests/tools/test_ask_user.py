...
async def test_ask_user_auto_auto_dismiss():
    """In auto mode, auto-dismiss without wire or tool_call context."""
    tool = AskUserQuestion()
    tool.bind_auto(is_auto=lambda: True)

    # Deliberately do NOT set wire or tool_call — auto should short-circuit.
    wire_token = _current_wire.set(None)
    try:
        ...
        parsed = json.loads(result.output)
        assert parsed["answers"] == {}
        assert "auto" in parsed.get("note", "").lower()
    finally:
        _current_wire.reset(wire_token)
...
async def test_ask_user_unbound_falls_through():
    ...
    tool = AskUserQuestion()
    # Do NOT call bind_auto — _is_auto stays None
...
async def test_ask_user_auto_dynamic_toggle():
    """When auto is toggled off dynamically, tool falls through to normal flow."""
    auto_state = {"enabled": True}
    tool = AskUserQuestion()
    tool.bind_auto(is_auto=lambda: auto_state["enabled"])

    # First call: auto on -> auto-dismiss
    result = await tool(_make_params())
    assert not result.is_error
    assert isinstance(result.output, str)
    ...
    # Second call: auto off -> needs wire (which isn't set -> error)
    wire_token = _current_wire.set(None)
    tool_call = ToolCall(
        id="tc-toggle",
        function=ToolCall.FunctionBody(name="AskUserQuestion", arguments=None),
...
async def test_ask_user_yolo_only_does_not_dismiss():
    """Yolo without auto must NOT auto-dismiss — the user is still at the terminal.
    ...
    """
    tool = AskUserQuestion()
    tool.bind_auto(is_auto=lambda: False)
